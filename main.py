...
def infer_image():
    ...
    if 'file' not in request.files:
        return jsonify(status=False, data="Please add an Image", message="Image Error")
    file = request.files.get('file')
    if not file:
        return jsonify(status=False, data="Image Not Valid", message="Image Error")
    ...
    if(not isImgAnAppleLeaf(img_small)):
        return jsonify(status=False, data="Image is not a leaf", message="Image Error")
    else:
